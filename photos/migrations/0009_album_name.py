...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        # copy the album name for all users
        for album in orm.Album.objects.all():
            for membership in album.memberships.all():
                membership.album_name = album.name
                membership.save(update_fields=['album_name'])

    def backwards(self, orm):
        for album in orm.Album.objects.all():
            if album.memberships.count() == 0:
                # memberless album
                album.name = str(album.id)
            else:
                if orm.AlbumMember.objects.filter(album=album, user__pk=album.creator.id).exists():
                    # try copying the creator's album name first
                    membership = album.memberships.get(user__pk=album.creator.id)
                else:
                    # copy the album name from any member
                    membership = album.memberships.first()
                album.name = membership.album_name
    ...
    models = {
        ...
        u'phone_auth.user': {
            ...
            'avatar_file': ('django.db.models.fields.CharField', [], {'default': "'s3:shotvibe-avatars-01:default-avatar-0063.jpg'", 'max_length': '128'}),
        ...
        u'photos.pendingphoto': {
            ...
            'photo_id': ('django.db.models.fields.CharField', [], {'default': "'9eb086c998291264fd12c155a472ac7e184b81f7704871d97c970055cd656091'", 'max_length': '128', 'primary_key': 'True'}),
